...
public partial class Game : Node2D
{
    ...
    public override void _Ready()
    {
        ...
        AddChild(_enemies);
        _world = new WorldGenerator { Name = "WorldGenerator" };
        AddChild(_world);
        _world.Setup(_enemies);
        _world.CoinCollected += OnCoin;
        _world.ShieldCollected += OnShield;
        _world.HazardHit += OnHazard;
        _player = new Player { Name = "Player", Position = new Vector2(180, GroundY - 35) };
        AddChild(_player);
    ...
    public override void _PhysicsProcess(double deltaValue)
    {
        ...
        _hud.UpdateStats(
            ...
            Score.Coins,
            Score.Multiplier(),
            DifficultyCurve.TierForSteps(_steps),
    ...
    private void WireEnemies()
    {
        ...
        foreach (Node child in _enemies.GetChildren())
        {
            if (child is not Enemy enemy || enemy.HasMeta("maryou_wired"))
                continue;
    ...
    private void OnEnemyStomp(Enemy _)
    {
        Score.AddStomp();
        if (IsInstanceValid(_player))
            _player!.Shake(5, 0.1f);
        Juice(0.06f, 0.88f);
    }
    ...
    private void TakeDamage()
    {
        ...
        if (_player.TakeHit())
        {
            ...
            else
            {
                ...
                _player.Velocity = new Vector2(DifficultyCurve.SpeedForSteps(_steps) * 0.75f, -360);
                Juice(0.08f, 0.82f);
            }
        }
        else
        ...
        SceneTreeTimer timer = GetTree().CreateTimer(1.15, true, false, true);
    ...
    private void Juice(float duration, float timeScale)
    {
        ...
        SceneTreeTimer timer = GetTree().CreateTimer(duration, true, false, true);
    ...
    public override void _Draw()
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            DrawTextureRectRegion(
                _backgrounds,
                new Rect2(x, -40, BackdropWidth, 720),
                new Rect2(sourceX, 0, 256, 256));
        }
    }
}
