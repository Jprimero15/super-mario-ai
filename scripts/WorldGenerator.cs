...
public partial class MaryouWorldGenerator : Node2D
{
    ...
    {
        _enemyRoot = enemies;
        // Each run gets a fresh seed so repeated runs are replayable only when the seed is recorded.
    ...
    public void GenerateUntil(float playerX, int distanceSteps)
    {
        ...
        while (_generatedTo < target)
        {
            _generatedTo++;
            GenerateChunk(_generatedTo, distanceSteps);
        }
        Prune(playerX);
    }
    ...
    private void GenerateChunk(int chunkIndex, int distanceSteps)
    {
        ...
        var root = new MaryouChunk { Name = $"Chunk_{chunkIndex}" };
        AddChild(root);
        _activeChunks[chunkIndex] = root;
        ...
        float holeChance = MaryouDifficultyCurve.HoleChance(distanceSteps);
        ...
        for (int i = 0; i < 20; i++)
        {
            ...
            {
                float width = rng.RandiRange(32, 64);
                var hole = new Rect2(x, GroundY, width, Tile);
                if (Safe(hole, occupied, holes, 42))
                {
                    holes.Add(hole);
                    safeGapUntil = hole.End.X + 96;
                }
            }
        }
        ...
        for (int i = 0; i < 20; i++)
        {
            var tile = new Rect2(startX + i * Tile, GroundY, Tile, Tile);
            bool blocked = false;
            ...
            {
                AddSolid(root, tile);
                root.Solids.Add(tile);
            }
        }

        root.Holes.AddRange(holes);
        foreach (var hole in holes)
            AddHoleWarning(root, hole);

        if (chunkIndex > 0)
        {
            int decorCount = rng.RandiRange(2, 4);
            for (int i = 0; i < decorCount; i++)
                AddEnvironment(root, startX + rng.RandiRange(2, 19) * Tile, rng.Randf() < .58f ? 0 : 160);
        }

        int pipeAttempts = 1 + distanceSteps / 500;
        int pipesPlaced = 0;
        for (int i = 0; i < pipeAttempts; i++)
        {
            if (rng.Randf() > MaryouDifficultyCurve.PipeChance(distanceSteps))
                continue;

            var pipe = new Rect2(
                startX + rng.RandiRange(9, 18) * Tile,
                GroundY - rng.RandiRange(2, 3) * Tile,
                Tile,
                rng.RandiRange(2, 3) * Tile);

            if (pipe.Position.X < safeGapUntil || !Safe(pipe, occupied, holes, 42))
                continue;
        ...
        if (chunkIndex > 0 && pipesPlaced == 0 && rng.Randf() < .78f)
        {
            ...
            if (Safe(fallback, occupied, holes, 42))
            {
                occupied.Add(fallback);
                root.Pipes.Add(fallback);
                AddSolid(root, fallback);
                AddHazard(root, fallback);
            }
        }
        ...
        for (int i = 0; i < coinCount; i++)
        {
            ...
            if (Safe(new Rect2(pos - new Vector2(15, 15), new Vector2(30, 30)), occupied, holes, 10))
                AddCollectible(root, pos, "coin");
        }
        ...
        if (distanceSteps >= 650 && rng.Randf() < .22f)
        {
            ...
            if (Safe(new Rect2(pos - new Vector2(15, 15), new Vector2(30, 30)), occupied, holes, 10))
                AddCollectible(root, pos, "shield");
        }

        int count = MaryouDifficultyCurve.EnemyCount(distanceSteps);
        if (chunkIndex == 0)
            count = 0;
        ...
        for (int i = 0; i < count; i++)
        {
            for (int attempt = 0; attempt < 8; attempt++)
            {
                ...
                if (!Safe(enemyRect, occupied, holes, 48))
                    continue;

                var enemy = new MaryouEnemy { Position = new Vector2(enemyX, GroundY - 40) };
                enemy.Setup(MaryouDifficultyCurve.EnemyKind(distanceSteps, i));
                _enemyRoot!.AddChild(enemy);
                occupied.Add(enemyRect);
                break;
            }
        }

        // Carry only hazards from this chunk into the next chunk. This prevents seam clusters
        // without treating normal ground tiles as hazards.
        _previousBoundaryHazards.Clear();
        foreach (var hole in holes)
            _previousBoundaryHazards.Add(hole);
        foreach (var pipe in root.Pipes)
            _previousBoundaryHazards.Add(pipe);
        for (int i = 0; i < occupied.Count; i++)
        {
            Rect2 item = occupied[i];
            if (item.Position.X >= startX && item.Position.X < startX + ChunkWidth)
                _previousBoundaryHazards.Add(item);
        }
    }
    ...
    private void AddSolid(Node2D parent, Rect2 rect)
    {
        var body = new StaticBody2D
        {
            CollisionLayer = 2,
            CollisionMask = 1,
            Position = rect.Position + rect.Size * .5f
        };
        ...
        if (rect.Position.Y < GroundY && rect.Size.Y > Tile)
        {
            ...
            {
                Texture = _pipeTexture,
                TextureFilter = CanvasItem.TextureFilterEnum.Nearest,
                Scale = new Vector2(rect.Size.X / 32, rect.Size.Y / 96)
            };
    ...
    private void AddHoleWarning(Node2D parent, Rect2 hole)
    {
        var sprite = new Sprite2D
        {
            Texture = _obstacles,
            RegionEnabled = true,
            RegionRect = new Rect2(200, 8, 58, 60),
            TextureFilter = CanvasItem.TextureFilterEnum.Nearest,
            Position = new Vector2(hole.Position.X + hole.Size.X * .5f, GroundY - 18),
            Scale = new Vector2(hole.Size.X / 58, .72f)
        };
    ...
    private void AddEnvironment(Node2D parent, float x, float sourceY)
    {
        ...
        if (sourceY == 0)
            sprite.RegionRect = (int)(x / Tile) % 3 == 0 ? new Rect2(0, 0, 88, 142) : new Rect2(160, 0, 96, 126);
        else
            sprite.RegionRect = new Rect2(256, 0, 104, 126);
        sprite.Position = new Vector2(x, GroundY - 63);
        parent.AddChild(sprite);
    }

    private void AddHazard(Node2D parent, Rect2 pipe)
    {
        var area = new MaryouHazard();
        float height = Mathf.Max(10, pipe.Size.Y - 20);
        area.Position = pipe.Position + new Vector2(pipe.Size.X * .5f, pipe.Size.Y * .5f + 10);
        area.Setup(new Vector2(pipe.Size.X + 8, height));
        area.HitPlayer += () => EmitSignal(SignalName.HazardHit);
        parent.AddChild(area);
    }

    private void AddCollectible(Node2D parent, Vector2 position, string kind)
    {
        var item = new MaryouCollectible { Position = position };
        item.Setup(kind);
        item.Collected += OnCollectible;
    ...
    {
        if (kind == "shield")
            EmitSignal(SignalName.ShieldCollected);
        else
    ...
    private void Prune(float playerX)
    {
        ...
        {
            if (IsInstanceValid(_activeChunks[key]) && key * ChunkWidth + ChunkWidth < pruneBefore)
            {
                _activeChunks[key].QueueFree();
        ...
        foreach (Node child in _enemyRoot.GetChildren())
            if (IsInstanceValid(child) && child is Node2D node && node.Position.X < pruneBefore)
                node.QueueFree();
    }
}
