...
public partial class MaryouEnemy : CharacterBody2D
{
    ...
    private const float Gravity = MaryouPlayer.SharedGravity;
    private int _kind;
    private float _speed = 70, _size = 30, _baseY, _phase, _patrolOrigin;
    private bool _defeated, _shielded;
    private Area2D? _hitArea;
    private Area2D? _stompArea;
    ...
    public void Setup(int enemyKind)
    {
        ...
        _animatedSprite = new AnimatedSprite2D
        {
            ...
            Animation = $"kind_{_kind}",
            Position = new Vector2(0, -2),
            TextureFilter = CanvasItem.TextureFilterEnum.Nearest
        };
        AddChild(_animatedSprite);

        // Side/body contact box. The separate stomp box above the enemy owns stomp detection.
        ...
        _stompArea.BodyEntered += OnStompAreaEntered;
        AddChild(_stompArea);

        _baseY = Position.Y;
        _patrolOrigin = Position.X;
        _phase = Position.X * .02f;
        _hopperState = HopperState.Idle;
        _hopperTimer = 0;
        ZIndex = 7;
    ...
    public void Tick(double deltaValue, float playerX)
    {
        ...
        {
            case 0:
                Velocity = new Vector2(direction * _speed, Velocity.Y + Gravity * delta);
                break;
            case 1:
                _phase += delta * 5;
                Velocity = new Vector2(direction * _speed * .65f, (_baseY - Mathf.Abs(Mathf.Sin(_phase)) * 75 - Position.Y) * 8);
                break;
            case 2:
                Velocity = new Vector2(direction * _speed * (Mathf.Abs(playerX - Position.X) < 240 ? 1.79f : 1), Velocity.Y + Gravity * delta);
                break;
            case 3:
                Velocity = new Vector2(direction * _speed * .8f, Velocity.Y + Gravity * delta);
                break;
            case 4:
                _phase += delta * 4;
                Velocity = new Vector2(Mathf.Sin(_phase) * _speed + direction * _speed * .45f, (_baseY - 30 + Mathf.Sin(_phase * 1.7f) * 38 - Position.Y) * 7);
                break;
            case 5:
                TickHopper(delta, playerX, direction);
                break;
        }
    ...
    {
        float distance = Mathf.Abs(playerX - Position.X);
        _hopperTimer = Mathf.Max(0, _hopperTimer - delta);

        switch (_hopperState)
        {
            case HopperState.Idle:
                Velocity = new Vector2(direction * _speed * .55f, Velocity.Y + Gravity * delta);
                if (IsOnFloor() && distance < 300 && _hopperTimer <= 0)
                {
                    _hopperState = HopperState.Windup;
                    _hopperTimer = .16f;
            ...
            case HopperState.Windup:
                ...
                if (_hopperTimer <= 0)
                {
                    Velocity = new Vector2(Velocity.X, -520);
                    _hopperState = HopperState.Hop;
                }
                break;
            case HopperState.Hop:
                ...
                {
                    _hopperState = HopperState.Cooldown;
                    _hopperTimer = .42f;
                }
                break;
    ...
    {
        if (_defeated || body is not MaryouPlayer player)
            return;
        ...
        if (_shielded)
        {
            player.Velocity = new Vector2(player.Velocity.X, -330);
            player.TakeHit();
            return;
        }

        Defeat();
        player.Velocity = new Vector2(player.Velocity.X, -400);
        EmitSignal(SignalName.Stomped, this);
    }
    ...
    private void Defeat()
    {
        ...
        _stompArea?.SetDeferred("monitoring", false);
        Velocity = new Vector2(Velocity.X * .2f, -330);
