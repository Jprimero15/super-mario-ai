...
func _draw() -> void:
	...
	for solid in solids:
		draw_rect(solid, Color("#5b3f2b"))
		draw_rect(Rect2(solid.position, Vector2(solid.size.x, 9.0)), Color("#78ad52"))
		draw_line(solid.position, Vector2(solid.end.x, solid.position.y), Color("#9bca6b"), 2.0)
