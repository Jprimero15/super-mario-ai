...
/** Fast grid-indexed collision handler that can accept procedural world chunks. */
class CollisionHandler(solids: List<Rectangle>, private val tileSize: Float) {
    ...
    private inline fun forEachNearbyTile(bounds: Rectangle, action: (Rectangle) -> Unit) {
        ...
        for (tx in minTx..maxTx) for (ty in minTy..maxTy) tileGrid[tx to ty]?.let(action)
    }
    ...
    fun resolveX(bounds: Rectangle, velocity: Vector2) {
        ...
        if (vx > 0f) {
            ...
            forEachNearbyTile(bounds) { tile -> if (bounds.overlaps(tile) && tile.x < nearestRight) nearestRight = tile.x }
            if (nearestRight != Float.POSITIVE_INFINITY) { bounds.x = nearestRight - bounds.width; velocity.x = 0f }
        } else {
            var nearestLeft = Float.NEGATIVE_INFINITY
            forEachNearbyTile(bounds) { tile -> if (bounds.overlaps(tile) && tile.x + tile.width > nearestLeft) nearestLeft = tile.x + tile.width }
            if (nearestLeft != Float.NEGATIVE_INFINITY) { bounds.x = nearestLeft; velocity.x = 0f }
        }
    }
    ...
    fun resolveY(bounds: Rectangle, velocity: Vector2): Boolean {
        ...
        if (vy < 0f) {
            ...
            forEachNearbyTile(bounds) { tile -> if (bounds.overlaps(tile) && tile.y + tile.height > highestTop) highestTop = tile.y + tile.height }
            if (highestTop == Float.NEGATIVE_INFINITY) return false
            bounds.y = highestTop; velocity.y = 0f; return true
        }
        var lowestBottom = Float.POSITIVE_INFINITY
        forEachNearbyTile(bounds) { tile -> if (bounds.overlaps(tile) && tile.y < lowestBottom) lowestBottom = tile.y }
        if (lowestBottom != Float.POSITIVE_INFINITY) { bounds.y = lowestBottom - bounds.height; velocity.y = 0f }
        return false
    }
