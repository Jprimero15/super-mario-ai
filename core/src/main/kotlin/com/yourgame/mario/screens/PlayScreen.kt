...
class PlayScreen(private val game: MarioGame) : Screen {
    ...
    private fun draw(){
        Gdx.gl.glClearColor(.035f,.055f,.085f,1f);Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);viewport.apply();shapeRenderer.projectionMatrix=camera.combined;shapeRenderer.begin(ShapeRenderer.ShapeType.Filled)
        shapeRenderer.color=Color(.28f,.55f,.82f,1f);shapeRenderer.rect(camera.position.x-420f,0f,840f,WORLD_HEIGHT);shapeRenderer.color=Color(.47f,.72f,.88f,.22f);shapeRenderer.circle(camera.position.x-230f,305f,110f);shapeRenderer.circle(camera.position.x+190f,285f,135f);shapeRenderer.color=Color(.17f,.42f,.34f,.92f);shapeRenderer.circle(camera.position.x-180f,80f,100f);shapeRenderer.circle(camera.position.x+220f,70f,120f)
