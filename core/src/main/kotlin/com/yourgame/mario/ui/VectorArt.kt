...
object VectorArt {
    fun player(r: ShapeRenderer, b: Rectangle, facingRight: Boolean, big: Boolean, dead: Boolean) {
        ...
        r.color=Color(0f,0f,0f,.16f);r.ellipse(x+w*.12f,y-3f,w*.76f,7f)
        r.color=Color(.28f,.09f,.05f,1f);r.ellipse(x+w*.02f,y,w*.45f,h*.20f);r.ellipse(x+w*.53f,y,w*.45f,h*.20f)
        r.color=Color(.08f,.30f,.72f,1f);r.rect(x+w*.18f,y+h*.18f,w*.64f,h*.43f)
        r.color=Color(.88f,.08f,.06f,1f);r.rect(x+w*.08f,y+h*.40f,w*.84f,h*.32f)
        r.color=Color(1f,.70f,.48f,1f);r.ellipse(x+w*.16f,y+h*.55f,w*.68f,h*.36f)
        r.color=Color(.90f,.06f,.05f,1f);r.ellipse(x+w*.10f,y+h*.75f,w*.78f,h*.22f);r.rect(x+w*.22f,y+h*.77f,w*.58f,h*.13f)
        r.color=Color(.22f,.09f,.04f,1f);r.ellipse(x+w*.39f,y+h*.62f,w*.28f,h*.10f);r.ellipse(x+w*.52f,y+h*.70f,w*.08f,h*.06f)
        r.color=Color.BLACK;val eyeX=if(facingRight)x+w*.63f else x+w*.31f;r.circle(eyeX,y+h*.72f,w*.035f)
        if(big){r.color=Color(1f,1f,1f,.14f);r.rect(x,y,w,h)}
    }

    fun enemy(r: ShapeRenderer,b:Rectangle){val x=b.x;val y=b.y;val w=b.width;val h=b.height
        r.color=Color(.35f,.16f,.07f,1f);r.ellipse(x,y+h*.18f,w,h*.65f);r.color=Color(.72f,.31f,.10f,1f);r.ellipse(x+w*.10f,y+h*.35f,w*.80f,h*.50f)
        r.color=Color.WHITE;r.ellipse(x+w*.18f,y+h*.48f,w*.25f,h*.22f);r.ellipse(x+w*.57f,y+h*.48f,w*.25f,h*.22f);r.color=Color.BLACK;r.circle(x+w*.31f,y+h*.57f,w*.05f);r.circle(x+w*.69f,y+h*.57f,w*.05f)
    ...
    /** Translucent pill-like control: the world remains visible underneath. */
    fun button(r:ShapeRenderer,b:Rectangle,pressed:Boolean,accent:Color=Color(.12f,.16f,.22f,1f),alpha:Float=.48f){
        val lift=if(pressed)0f else 3f
        r.color=Color(0f,0f,0f,.16f*alpha/.48f);r.rect(b.x,b.y,b.width,b.height*.84f)
        r.color=Color(accent.r,accent.g,accent.b,alpha);r.rect(b.x,b.y+lift,b.width,b.height*.84f)
        r.color=Color(1f,1f,1f,.18f);r.rect(b.x+5f,b.y+b.height*.72f+lift,b.width-10f,2f)
    }

    fun pause(r:ShapeRenderer,b:Rectangle,pressed:Boolean){button(r,b,pressed,Color(.08f,.11f,.16f,1f),.55f);r.color=Color(1f,1f,1f,.92f);r.rect(b.x+b.width*.30f,b.y+b.height*.27f,b.width*.11f,b.height*.36f);r.rect(b.x+b.width*.59f,b.y+b.height*.27f,b.width*.11f,b.height*.36f)}
